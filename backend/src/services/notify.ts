...
export async function notify(event: NotifyEvent) { try { const payload = montarMensagem(event);
...
} catch (error) { console.error("❌ Erro ao disparar notificação:", event, error); } }

function montarMensagem(event: NotifyEvent) { switch (event.type) { case "BILHETE_CRIADO": return { userId: event.userId, title: "🎟️ Bilhete criado", body: Seu bilhete ${event.codigo} foi gerado com sucesso, url: "/meus-bilhetes", };
