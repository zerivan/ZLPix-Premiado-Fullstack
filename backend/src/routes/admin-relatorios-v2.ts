...
// 🔥 CORREÇÃO: pegar TODOS os usuários
const usuariosBase = await prisma.users.findMany({
...
// 🔥 garantir todos os usuários na lista
usuariosBase.forEach((u) => ensureRow(u.id));
...
const premiosDetalhados = premios.map((premio) => {
  ...
  return {
    ...
    nome: premio.user?.name || "Usuário sem nome",
...
return res.json({
  resumo: {
    ...
    premiosPagos,
    saquesPagos,
    lucroLiquido: arrecadado - premiosPagos, // 🔥 corrigido
  },
  usuarios,
