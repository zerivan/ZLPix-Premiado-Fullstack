...
export default function HistoricoModal({ onClose, items = [] as Bet[] }: { onClose: ()=>void; items?:Bet[] }) {
  return (
            ...
            items.map(b=>(
              <div key={b.id} style={{padding:10, borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  <div><strong>{b.dezenas.join(", ")}</strong></div>
                  <div className="small-muted">{b.data}</div>
                </div>
                <div className="small-muted">Valor: R$ {b.valor.toFixed(2)}</div>
              </div>
            ))
          }
        </div>
