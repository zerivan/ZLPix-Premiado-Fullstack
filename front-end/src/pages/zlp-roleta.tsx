...
export default function ZLPRoletaPage() {
  ...
  function resolveUserId() {
    ...
    try {
      ...
      return String(parsed?.id ?? parsed?.user?.id ?? parsed?.userId ?? parsed?._id ?? "");
    } catch {
  ...
  async function carregarSaldo() {
    ...
    try {
      ...
      setSaldo(normalizar(res.data?.saldo ?? res.data?.wallet?.saldo ?? res.data?.balance ?? 0));
    } catch (err) {
  ...
  async function handleResgatar() {
    ...
    try {
      ...
      const res = await api.post("/zlp/resgatar", {}, { headers: { "x-user-id": userId } });
      ...
      setMessage(err?.response?.data?.error || err?.response?.data?.message || "Erro ao resgatar bilhete");
    } finally {
      ...
      <div className="mx-auto w-full max-w-md rounded-3xl border border-blue-200/20 bg-gradient-to-br from-[#0b1e5b] via-[#0a2d82] to-[#051338] p-5 text-white shadow-[0_30px_120px_rgba(0,0,0,0.55)]">
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-center backdrop-blur-xl">
            ...
            <p className="mt-1 text-base font-extrabold text-white">{resultado ? resultado.label : "Gire para ganhar ZLP"}</p>
          </div>
          ...
          {message ? (
            <p className="rounded-xl border border-white/15 bg-white/10 px-3 py-2 text-center text-sm text-blue-50">{message}</p>
          ) : null}
        </div>
      </div>
    </div>
