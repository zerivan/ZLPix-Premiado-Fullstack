...
export default function ZLPRoletaPage() {
  ...
  // ✅ CORREÇÃO: alinhamento com ponteiro (sem alterar layout)
  function calcularSetor(grau: number) {
    const tamanho = 360 / setores.length;
    const normalizado = ((grau % 360) + 360) % 360;
    const anguloPonteiro = (360 - normalizado) % 360;
    const index = Math.floor(anguloPonteiro / tamanho);
  ...
  async function girar() {
    ...
    window.setTimeout(async () => {
      ...
      if (setor.bonus === "free-spin") {
        ...
        setMessage("🎉 Você ganhou um giro grátis! Gire novamente.");
      }
